...
> **Architecture Decision (2026-02-27)**: Episodic memory lives exclusively in
...
|---------|------------------|-----|
| **PII in memories** | No automatic PII detection or redaction. Observer extracts whatever LLM returns. | Need: PII filter in Observer pipeline before persist. |
| **Credential leakage** | Tool results may contain tokens, keys. `tool_result` type has 7-day decay + session scope. | Partial mitigation via short TTL. Need: explicit secret detection + immediate redaction. |
| **Cross-session leakage** | `session_id = NULL` memories are visible to all future sessions. | By design for knowledge. Risk: session-specific sensitive context promoted to cross-session semantic memory by Reflector. Need: sensitivity classification in Observer. |
| **Memory deletion (right to forget)** | `is_active = 0` soft-deletes exclude from retrieval. PITR still shows historical state within retention window. | Soft delete is sufficient for retrieval isolation. True erasure requires PITR retention expiry (configurable). |
| **Audit trail vs privacy** | `context_snapshot` records what agent saw, including memory content. | Tension: audit needs full record, privacy needs redaction. Resolution: snapshot stores memory_ids, not content. Content looked up at audit time (respects current `is_active` state). |

> **Design gap**: The Observer pipeline currently has no sensitivity filter.
> A pre-persist hook that classifies extracted memories (e.g., contains PII,
> contains credentials, session-specific-only) would allow:
> - Auto-redact PII before storage
> - Force `session_id` on sensitive memories (prevent cross-session promotion)
> - Block credential-containing content from entering `memories` at all
...
> **Design Decision (2026-02-27)**: Confidence decay is **query-time only**.
> The `memories` table stores `initial_confidence` (immutable after write).
> `effective_confidence` is computed in every retrieval query. Governance does
> NOT mutate the `confidence` column. This is stateless and idempotent.
>
> **Schema change required**:
> - Rename `confidence` → `initial_confidence` in MemoryRecord
> - Retriever computes in SQL: `initial_confidence * EXP(-TIMESTAMPDIFF(DAY, observed_at, NOW()) / :half_life)`
> - Governance `_apply_decay()` removed
> - Governance only deactivates memories where effective_confidence < threshold
...
#### Source Trust Tiers (Deferred)

> **Deferred (2026-02-27)**: The system uses a single `half_life_days=30` for all
> memory types. This is sufficient to validate the decay mechanism end-to-end.
> Trust tiers will be reconsidered after collecting production data on memory
> quality distribution by source. The schema extension (adding `trust_tier` column)
> is backward-compatible and can be done incrementally.

Design target for future implementation:
...
> **Session summary generation**: When a session closes, the Reflector automatically
> generates a session summary and stores it as a semantic memory (`type=semantic`,
> `session_id=NULL`). This is the bridge between episodic and cross-session recall —
> raw events stay in `conversation_events` (session-scoped), but the distilled
> summary becomes permanent knowledge accessible to all future sessions.
...
CREATE TABLE memories (
  ...
  content          TEXT NOT NULL,
  confidence       FLOAT DEFAULT 0.75,     -- to be renamed initial_confidence (see B2)
...
> **Open issue**: No unified entry point when a query needs both knowledge and
> events. Caller must invoke both retrievers. Scoring weights differ.
...
MatrixOne's Python UDF enables pushing computation to the data (quality scoring, entity extraction). Not yet implemented — all scoring currently runs application-side.
...
> **Design Decision (2026-02-27)**: Replace O(n²) Python-side clustering with
> DB-side L2_DISTANCE nearest-neighbor queries. O(n) DB queries via IVF-flat index.
...
(test2.0)  xupeng@macpro  ~/github/mo-agent-engine   dev ±✚ 
...
(test2.0)  xupeng@macpro  ~/github/mo-agent-engine   dev ±✚ 
(test2.0)  xupeng@macpro  ~/github/mo-agent-engine   dev ±✚ 
(test2.0)  xupeng@macpro  ~/github/mo-agent-engine   dev ±✚  cat docs/design/memory-architecture.md
# Memory Architecture
