...
func NewSimpleEvictHolder(ctx ...interface{}) base.IEvictHolder {
...
func (holder *SimpleEvictHolder) Enqueue(node *base.EvictNode) {
	holder.Queue.Put(node)
}

func (holder *SimpleEvictHolder) Dequeue() *base.EvictNode {
	r, ok, _ := holder.Queue.Get()
	if !ok {
		return nil
	}
	return r.(*base.EvictNode)
}
